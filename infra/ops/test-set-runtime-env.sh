...
tmp="$(mktemp -d)"
cd "$tmp"
printf 'A=1\nDATABASE_LISTEN_URL=old\nB=2\n' > .env.runtime
eval "$(sed -n '/^  set_runtime_env() {/,/^  }/p' "$startup")"
set_runtime_env DATABASE_LISTEN_URL 'postgresql://u:p%40ss@host:5432/db?sslmode=require&application_name=a|b\\c'
set_runtime_env NEW_KEY 'x=y'
expected='A=1
DATABASE_LISTEN_URL=postgresql://u:p%40ss@host:5432/db?sslmode=require&application_name=a|b\\c
B=2
NEW_KEY=x=y'
