...
export function createDockerClient(connection: DockerConnection | RemoteDockerConnection): Docker {
  ...
  if (connection.dockerSocketPath) return new Docker({ socketPath: connection.dockerSocketPath });
  return new Docker();
}
...
export class DockerContainerRuntime implements ContainerRuntime {
  ...
  // Archive read, not exec: works on a stopped container too.
  async readFile(
    containerId: string,
    path: string,
    maxBytes: number,
  ...
  // True only when the start-up window ended healthy; any other exit from "starting" is false.
  ...
  private async inspect(containerId: string): Promise<Docker.ContainerInspectInfo | null> {
    try {
