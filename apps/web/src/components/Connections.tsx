  ...
  return (
    <section id="connections" aria-labelledby="connections-title" className="py-16 sm:py-24">
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
        ...
        <ul className="sd-stagger grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 gap-3 sm:gap-4">
          {LANDING_APPS.map((app) => (
            <li
              key={app.slug}
              className="lift flex flex-col items-center gap-2.5 sm:gap-3 rounded-2xl sm:rounded-[20px] border border-sand-light bg-white px-2 py-[18px] sm:px-3 sm:py-6"
            >
        ...
        <p className="sd-reveal mt-7 sm:mt-8 flex flex-col sm:flex-row items-center justify-center gap-2.5 text-center text-sm sm:text-base text-espresso-light leading-relaxed">
          <span className="rounded-full bg-cream-dark px-3.5 py-1 text-sm text-espresso">מעל <CountUp label={CATALOG_SIZE_LABEL} /> אפליקציות נוספות</span>
