...
import { SETTINGS_PATH, settingsReturnPath } from "@/lib/billing/urls";
...
export default async function PayPage({
  ...
  if ("_ptxn" in query.data) {
    ...
    return <PaddleCheckout {...paddle} transactionId={null} email={null} successPath={SETTINGS_PATH} exitPath={SETTINGS_PATH} />;
  }
  ...
  if (checkout.status !== "pending") redirect(settingsReturnPath(checkout.status === "completed" ? "success" : "failed", checkout.id));
  ...
  return (
    <PaddleCheckout
      ...
      email={user.email}
      successPath={settingsReturnPath("success", checkout.id)}
