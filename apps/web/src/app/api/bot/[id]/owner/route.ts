...
import { BotIdParamsSchema, OwnerIdentityBodySchema } from "@/lib/bots/schemas";
...
export async function GET(
  ...
  return authenticatedHandler({}, async ({ userId }) => {
    const owner = await botService.getOwnerIdentity(userId, parsed.data.id);
...
export async function PATCH(
  ...
  return authenticatedHandler(
    { bodySchema: OwnerIdentityBodySchema },
    async ({ userId, body }) => {
      const owner = await botService.updateOwnerIdentity(userId, parsed.data.id, body);
      return NextResponse.json(owner);
    },
  )(req);
}
