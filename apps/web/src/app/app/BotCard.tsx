...
function PhoneRow({ accountId }: { accountId: string }) {
  ...
  const onCopy = async () => {
    try {
      ...
      // Clipboard blocked (insecure context, permissions). Silent — the number is visible.
    }
  };
  return (
    <p className="text-sm text-espresso-light mb-6 flex items-center gap-2">
      <span>מחובר למספר</span>
      <span dir="ltr" className="font-mono text-espresso">{display}</span>
      <button
        type="button"
        onClick={onCopy}
        aria-label={copied ? "המספר הועתק" : "העתקת המספר"}
        className="rounded-md p-1 text-espresso-light hover:text-espresso hover:bg-cream-dark focus:outline-none focus-visible:ring-2 focus-visible:ring-terra transition"
      >
  ...
  return (
    <div className="space-y-3">
      <a
        ...
        className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-terra text-white font-medium hover:bg-terra-light transition"
      >
        <WhatsAppIcon />
        <span>פתחו ב-WhatsApp ושלחו הודעה</span>
      </a>
      <p className="text-xs text-espresso-light leading-relaxed max-w-md">
        ההודעה הראשונה עשויה להגיע אחרי כ-30–40 שניות — הסוכן עולה ברגעים אלו. ההודעות הבאות יענו מיידית.
      </p>
    </div>
  );
}
