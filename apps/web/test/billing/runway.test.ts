...
test("no estimate before a day of pace, without any spend, or when the grant expires first", () => {
  const fresh = grant({ usedCredits: 50, expiresAt: daysAfter(30) });
  assert.equal(runwayDays([fresh], 950, daysAfter(0.5)), null);
  assert.equal(runwayDays([grant({ expiresAt: daysAfter(30) })], 1000, daysAfter(3)), null);
