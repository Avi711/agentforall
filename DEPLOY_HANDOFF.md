...
- The four `default-allow-*` firewall rules still exist but are inert behind the deny-all tie; delete by hand when convenient. SSH is IAP only now. Ops helper on the VM: `sudo agent-forall-admin GET /api/v1/admin/instances` (service token, frontend IP); the public site refuses private sources (S-14).
...
## Phase 2 step 8: orchestrator on its own VM (CUT OVER 2026-09-17 11:38-11:41 UTC; old VM stopped, not yet retired)
...
- Rollback while the old VM exists: `compose down` on the new VM, `delete-access-config` there, `control_plane_vm = "platform"`, apply, start `agent-forall`.
...
- Deploy recipe since the cutover: build → pin the digest in `infra/variables.tf` → `terraform apply -target=google_compute_instance.orchestrator` → `sudo google_metadata_script_runner startup` on VM `orchestrator` (compose recreates on an image or env change; a rotated cert or CA file recreates orchestrator + Caddy). `control_plane_vm` has no default on purpose: a tfvars without it must fail, never read as a rollback.
...
- Retirement list for the old VM, after a clean day: triage `/home/deploy/backups` on its boot disk (tarballs with WhatsApp sessions, env backups); manual final snapshots, then decide how long any snapshot of the boot disk may live (it holds `.env.runtime`, the CA root key and the mTLS client key); remove `prevent_destroy` from `google_compute_instance.platform` and `google_compute_disk.data`, `deletion_protection` off in a first apply, destroy the instance, both snapshot-policy attachments, the data disk and `google_compute_address.platform_internal` in a second. KEEP in `orchestrator.tf`: `google_compute_address.platform` (the live public IP), `google_service_account.platform` + its IAM (the new VM's identity), `google_compute_resource_policy.daily_snapshot` (every worker uses it); rename them with `moved {}` blocks then. Delete with the VM: `startup/orchestrator.sh`, `infra/ops/test-set-runtime-env.sh`, the `control_plane_vm` variable with both `dynamic "access_config"` blocks, the `dns.tf` ternary and both `stack_enabled` gates, `var.machine_type`, `var.disk_size_gb`, `var.orchestrator_internal_ip`; fix the references in `docs/security-todo.md`. The hosts row `agent-forall-vm` stays (57 destroyed rows point at it).
