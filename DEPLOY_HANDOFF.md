...
**openclaw-browser:** build from the repo root — the image bakes in `packages/openclaw-plugin-credit`, so the old `infra/images/openclaw-browser/` build context no longer works.
```bash
