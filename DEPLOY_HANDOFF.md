...
**2026-09-23 — everyone on credits.** The 8 pre-billing owners were moved onto the ledger (cursor at current spend, remaining budget granted as top-up `legacy:<userId>`, 0 for Asado) and every bot is now metered regardless of grants; admin "Add credits" works for anyone. Their keys still carry a `budget_duration` because LiteLLM 1.83.14 drops `budget_duration: null` on `/key/update`; it was set to `36500d` (resets 2126) instead, and `instances.litellm_budget_duration` says the same. Trial now means "no trial grant" (top-ups don't consume it); beta and enforcement-off users get the trial on their first bot; paying users never do.
...
- The `d99dec8d…` image above is OpenClaw 2026.7.1 plus the `agentforall-credit` plugin (`packages/openclaw-plugin-credit`, a `before_agent_reply` hook that answers with a Hebrew top-up message once LiteLLM `/key/info` reports `spend >= max_budget`; fails open on any lookup problem; 15s verdict cache). Verified on test bot קוקי2 (`openclaw-e43f2627-29d`, budget set to 0): plugin loads alongside the full bundled set. Do NOT set `plugins.allow` — on a live gateway it shrank the plugin set from 10 to 3, dropping memory-core. **Pending rollout:** `10802b7` (2026-09-14) changed the plugin's reply text (no more "monthly quota"; points to the dashboard); tenants still run the old text until a new openclaw-browser image is built and `rollout-plugin.sh --plugin agentforall-credit` runs.
...
- Runtime image refs live in `infra/variables.tf` and are rendered into the startup scripts under `infra/startup/`. Update them only after typecheck/test/build and a production smoke test creates and deletes a temporary bot.
...
- Deployed image: `litellm-gateway@sha256:5bab1ca78080eb26f88a5cd89c32bffaa0fbe799a135390cae1c175b50fc10e0` (revision `00010-z5j`, deployed 2026-09-02 with `gcloud run services update --image`, still LiteLLM v1.83.14-stable): the alias now resolves to **`gemini-3.8-flash`** (GA 2026-09-02, same pricing as 3.7). Smoke-tested from a tenant's own key: `x-litellm-model-api-base` names `gemini-3.8-flash`, tool call returned, streaming+tools returned Hebrew text, `x-litellm-response-cost` non-zero and `key/info.spend` moved after LiteLLM's async write (about 45 s). Rollback: route traffic to revision `00009-r8r` (`01f96fab…`, 3.7-flash). Previous entry: `01f96fab…` smoke-tested 2026-08-15 on 3.7-flash.
...
| `encryption-key` | 64-hex-chars; encrypts `instance_settings.gateway_token` and the secret leaves of `instance_settings.config`. ⚠️ Rotation requires re-encrypting all rows. |
...
over the volume's session (only pairing injects; an inject failure now undoes the pairing); `error` rows keep their gateway port and
count toward the per-user quota; the relay rate limit is keyed on the bot id; `TRUST_PROXY` takes a CIDR list. By hand on the VM:
`.env.runtime` `TRUST_PROXY=172.16.0.0/24` (backup `.env.runtime.bak-20260914-trustproxy`) — verified: requests via Caddy log the real
client IP, a bot hitting the relay directly logs its tenant-net address. Rollback: previous digest `9286095c…` + `TRUST_PROXY=true`.
...
## Phase 2 step 6: first worker VM (2026-09-16 20:00-21:00 UTC, orchestrator `ed5938a6`, worker image `agent-forall-worker-20260916-1700`)

- worker-1 (e2-highmem-2, 10.164.0.40, no public IP, tag `worker`) runs bots over dockerd mTLS on :2376; the orchestrator VM keeps its 15 bots. Terraform: NAT (2 IPs, dynamic 512-8192 ports), private zone `orchestrator.internal.` → 10.164.0.4 (promoted to static, imported), deny-all 65534 + tag rules, Private Google Access, worker SA `agent-forall-worker`, per-worker cert secrets. Applied 22 add / 7 change / 0 destroy after importing the network, subnet and address (the import order matters: without it the plan wanted to replace the LiteLLM Cloud SQL resources).
- Secrets created by `infra/ops/issue-control-plane-cert.sh`: `control-plane-ca-cert/key`, `orchestrator-client-cert/key`, `worker-1-server-cert/key`. The CA key is never fetched by a VM; rotation = a new CA and every leaf.
- The four `default-allow-*` firewall rules were deleted 2026-09-18 (CIS: no SSH/RDP from 0.0.0.0/0); only the five `agent-forall-*` rules remain. SSH is IAP only now. Ops helper on the VM: `sudo agent-forall-admin GET /api/v1/admin/instances` (service token, frontend IP); the public site refuses private sources (S-14).
- Found and fixed live: the nightly `docker image prune -af` had removed the pairing and Hermes images (no container references them between uses), so QR pairing would have failed with "No such image"; pulled back by hand, and the housekeeping now deletes only images that are neither in `/etc/agent-forall/pinned-images` nor behind a container, on every host. The Docker API cannot pull from GAR at all (no credential), so the orchestrator only asserts presence.
- Found by the acceptance reboot: a stopped container on a `no`-policy host was never re-adopted because the health monitor wrote a failure every 15 s and the reconciler skips rows touched within 30 s. Fixed in `77e43fda` (no health write when the container is not running; the reconciler ignores a Docker-restarting container); `ed5938a6` adds the pre-commit review's fixes (image checked before a rebuild removes the old container; an exhausted re-adopt budget marks the row stopped). First deploy of the step was `bacdc397`; rollback floor for the move code stays `d78e9796`.
- Acceptance: move בוב to worker-1 204/100 s, QR pairing through sidecar port 18000, recreate 61 s, dockerd restart kept the container up, reboot re-adopted (`restartsInWindow 1`), move back; bucket empty; 15/15. DB suites 28/28 + 5/5 on a throwaway Postgres on the worker.
- Alert filters now key VM metrics on `metadata.user_labels.app` (every VM); the log alert stays on the orchestrator VM; new NAT drop/allocation alert.

## Phase 2 step 7: `instance_settings` split (DEPLOYED 2026-09-17 09:34-09:44 UTC, orchestrator `6d33333e`)

- Migration 0021: `instance_settings` (1:1, FK cascade) takes `config` + `gateway_token`; `instances` keeps the rest. The loops read `FleetInstance` from `instances` alone (no decrypt); API paths join. Expected egress ~147 → 55-70 MB/day at 15 bots; check the Supabase chart the day after.
- Deploy order (the old image cannot read the new schema, nor the new image the old one): `docker compose stop orchestrator` → `npm run db:migrate` → roll the new image. API/dashboard/relay down ~2 min, bots untouched.
- Rollback, valid any time, orchestrator stopped: `ALTER TABLE instances ADD COLUMN config jsonb, ADD COLUMN gateway_token varchar(256); ALTER TABLE instances DISABLE TRIGGER trg_instances_updated_at; UPDATE instances i SET config = s.config, gateway_token = s.gateway_token FROM instance_settings s WHERE s.instance_id = i.id; ALTER TABLE instances ENABLE TRIGGER trg_instances_updated_at; ALTER TABLE instances ALTER COLUMN config SET NOT NULL, ALTER COLUMN gateway_token SET NOT NULL; DROP TABLE instance_settings; DELETE FROM drizzle.__drizzle_migrations WHERE created_at = 1789595838658;` then the previous image.
- Rehearsed 2026-09-17 on a throwaway Postgres 17 on the VM with a dump of prod (74 rows): forward, rollback, re-apply, per-id md5 of both columns identical each time; DB suites 31/31 + 5/5. Gate query on prod: 0 paired bots without a whatsapp channel, 0 NULLs.
- Found by the rehearsal: prod had no `update_updated_at()` function and no triggers (that part of 0000 never landed; fresh databases have them). 0021 installs the function (`CREATE OR REPLACE`) and the `updated_at` triggers on `instances` and `instance_settings`, so prod and fresh databases match; the rollback keeps them (harmless) and disables the `instances` trigger around its copy so `updated_at` is preserved (rehearsed).
- New rule: `PATCH /config` refuses to remove the `whatsapp` channel while the bot is paired or a pairing is in flight ("disconnect WhatsApp first"); the health monitor probes WhatsApp on `whatsapp_paired` alone. A row with corrupt settings is now visible to the loops (probed; `findById` fails loud).
- Deploy: orchestrator stopped 09:34:04, 0021 applied, `orchestrator@sha256:6d33333ed0ccabcc42ea6070c9cd4face633dcc76924b7f976f3d10b30bbabca` (Cloud Build `bdd97ead`) up 09:44:00 (8 of the 10 minutes were a wait for an approval, not work). After: 15/15 running, 0 warnings, `instances` 74 = `instance_settings` 74, ledger 22, both triggers present, all 15 `last_seen_at` fresh, admin list decrypts all 15. `.env` backup `.env.bak-20260917-step7`. Rollback floor for the orchestrator is this image unless the rollback SQL runs first.
- Not built: the in-memory fleet cache. Levers if the chart passes ~120 MB/day: `HEALTH_POLL_INTERVAL_MS` 15 s → 30 s, then Supabase Pro.

## Phase 2 step 8: orchestrator on its own VM (CUT OVER 2026-09-17 11:38-11:41 UTC; old VM RETIRED 2026-09-18)

- Phase 0 DONE 2026-09-17: `TELEGRAM_MANAGER_BOT_TOKEN` was the only hand-set key on the VM (key-name diff of `.env.runtime` against the startup script: empty otherwise); now secret `telegram-manager-bot-token`, fetched and re-synced by the startup script, value verified equal on the VM. Log alert no longer pinned to the old instance id.
- Found: the "orchestrator needs attention" alert had been disabled since 2026-09-16 09:48 and Terraform's in-place edits never reached it (condition `display_name` over Google's 100-character limit, the provider reported success anyway), so step 6's "host unreachable" text was never live. Recreated with a short name: policy `16141526285656610754`, enabled, full filter.
- Phase A DONE 2026-09-17: `worker-2` (e2-highmem-4, 10.164.0.41, 50 GB data disk, cert secrets `worker-2-server-cert/key`), host row pre-inserted `draining`, registered with 32089 MB five seconds after the orchestrator recreate.
- `infra/ops/drain-host.sh` (runs on the orchestrator VM; the API's dry run is the pre-flight; stops at the first bot that does not come back the way it left). Dry run 2026-09-17: 13 of 15 ready. Refused: the test bot until ~2026-09-18 10:00 UTC (this morning's move test left its rollback copy on worker-1), and `86d4416c` stuck in `awaiting_qr`.
- Found, not fixed: pairing expiry measures `updated_at`, which the health heartbeat refreshes every minute, so a running bot's pairing never expires by the sweep (`86d4416c` has waited ~3 weeks; its sidecar exited long ago). It clears only when the user reopens pairing. Needs its own fix with a failing test; until then cancel that pairing before moving the bot.
- `MOVE_SOURCE_RETENTION_MS` (orchestrator `16cd5b8c`, default 24 h, min 5 min; Terraform `move_source_retention_ms`): the rollback window of a move was a hardcoded 24 h, which also blocks a second move and a host's retirement. Set to 15 min for the drain, back to the default at the cutover. Manual snapshot `predrain-data-20260917` of the old data disk taken first.
- Drain DONE 11:00-11:23 UTC with `drain-host.sh`: 15/15 on `worker-2`, 0 failures, both WhatsApp bots still `paired` (the health monitor's channel probe is the check). Lesson, my mistake: the move API's `dryRun` is a real rehearsal (stops the bot, exports the volume, starts it again); a fleet-wide dry run at ~10:35 and the first ten drain moves bounced bots once more than needed. The script no longer calls it; `--list` only lists. The stale `awaiting_qr` on `86d4416c` was cancelled through the owner's cancel route.
- New VM `orchestrator` (e2-small, 10.164.0.5, 10 GB data disk, `startup/control-plane.sh`: one env render every boot, only control-plane images pinned, Caddy + socket proxy by digest, off `tenant-net`, host id `orchestrator` pre-inserted `draining`). `control_plane_vm` in tfvars drives the public IP, the `orchestrator.internal` record (TTL 60) and which VM starts its stack; both startup scripts refuse to start the stack when they are not the control plane. `caddy_data` copied ahead, so the public certificate was there when the IP arrived.
- Cutover: gates 0/0 → `compose down` + stop old VM + `delete-access-config` → tfvars `control_plane_vm = "orchestrator"` → one saved plan (3 in-place changes) → startup rerun on the new VM. Down 11:38:25-11:41:25. After: public `/health` 200 with valid TLS on 34.90.58.155, 15/15 running, 0 warnings, both workers resolve 10.164.0.5 and re-registered, next plan empty.
- Pre-commit reviews (code + infra) applied 2026-09-17; orchestrator `orchestrator@sha256:2a35a7a2e6ee27b4d8711610cc933fec657d77d5e8092ece1cb0ef59f14e0563` rolled 11:57 UTC with the recipe below (only the orchestrator container recreated, 15/15, 0 warnings). It adds: a pairing never starts on a bot whose whatsapp channel is gone (re-checked after the claim, rolled back to `failed`).
- Deploy recipe since the cutover: build → pin the digest in `infra/variables.tf` → `terraform apply -target=google_compute_instance.orchestrator` → `sudo google_metadata_script_runner startup` on VM `orchestrator` (compose recreates on an image or env change; a rotated cert or CA file recreates orchestrator + Caddy).
- Open: user checks (dashboard, one Telegram link, one pairing QR); watch the e2-small's memory for a day (container limits add up to ~1.4 GB of 2 GB).
- Old VM `agent-forall` RETIRED 2026-09-18: instance, boot + data disk, `agent-forall-internal` (10.164.0.4) and both snapshot attachments destroyed (deletion protection off by gcloud first, then one apply: 5 destroyed, startup-script metadata updated in place on `orchestrator`; next plan empty, public `/health` 200). Removed with it: `infra/orchestrator.tf`, `startup/orchestrator.sh`, `ops/test-set-runtime-env.sh`, the `control_plane_vm` switch with both gates, `var.machine_type`, `var.disk_size_gb`, `var.orchestrator_internal_ip`. Kept under their old `platform` names (no state moves): the public IP and the service account in `orchestrator-vm.tf`, `daily_snapshot` in `workers.tf`. The hosts row `agent-forall-vm` stays (destroyed rows point at it). No rollback target exists any more: a bad control-plane deploy rolls back by re-pinning the previous orchestrator digest.
- Old-disk snapshots: the 28 scheduled ones and `predrain-data-20260917` deleted. `retire-boot-20260918` + `retire-data-20260918` kept; the boot one holds `.env.runtime`, the CA root key and the mTLS client key and `/home/deploy/backups` (not triaged): DELETE BOTH BY 2026-10-18. Five older manual snapshots of the same disks still exist (`pre-openclaw-8-2-20260902-1741`, `prereboot-boot|data-20260913-2048|2235`), owner decision pending.

## Pairing expiry fix (DEPLOYED 2026-09-17 12:32 UTC, orchestrator `418d3aaa`, migration 0022)

- Bug: the stale-pairing sweep measured `updated_at`, which every health heartbeat refreshes, so a running bot's pairing never expired (one bot sat in `awaiting_qr` ~3 weeks). Fix: `instances.pairing_started_at`, stamped at the claim; the sweep measures from it, NULL counts as stale. `PAIRING_STALE_THRESHOLD_MS` is now a 1 h backstop for dead sessions (the sidecar's 10 min idle timer is the live timeout; a 15 min hard cap could cut an active scan). One failing bot no longer aborts the sweep.
- Deploy: `db:migrate` first (additive nullable column, old image unaffected), then the image with the step 8 recipe (only the orchestrator container recreated). After: 15/15, 0 warnings, no bot in `awaiting_*`. DB suites 31/31 + 5/5 on a throwaway Postgres on worker-1 (deleted).
- Test date bomb defused: a fixed 2026-09-10 seed crossed the WhatsApp Cloud sweeper test's 7-day cutoff at 10:00 UTC today; DB test seeds are now relative to the clock.
- Known, minor, not fixed: after a sidecar dies while the row is still `awaiting_qr`, the first retry answers `already_active` without a sidecar; the second retry works.
- Next: step 9, fleet updates through the orchestrator (brief reviewed: plugins converge offline in `prepareState` from the image's tarballs, system recreate with a volume snapshot to `snapshots/` before every migration, `GET .../verify`, one script; `rollout-plugin.sh` goes away).

## Step 9: fleet updates through the orchestrator (DEPLOYED 2026-09-17 14:33 UTC, orchestrator `8998b67b`)

- Why: bots live on workers only the orchestrator reaches; the old `recreate-tenants.sh` and `rollout-plugin.sh` drove the local Docker and found nothing on the control-plane VM.
- Every rebuild (`prepareState`: recreate, restart or start of an off-image bot, restore, move target) now also installs every plugin tarball the image ships (`/opt/agentforall/plugins/*.tgz`) into the volume, offline in a one-off container. A plugin rollout = bump the bot image, then `recreate-tenants.sh`. `rollout-plugin.sh` is deleted.
- `POST /api/v1/admin/instances/:id/recreate` (system actor, no impersonation) and `GET .../verify` → `{ status, image, onCurrentImage, running, checks }`; checks: expected plugins loaded (`plugins list --json`), WhatsApp plugin version = core, `config validate`, orchestrator-owned settings in place. 409 while the bot is under an operation; a wedged bot yields failed checks, never a 500.
- Before a rebuild onto ANOTHER image the stopped volume streams to `gs://agent-forall-moves/snapshots/<id>/<iso>.tar` (event `instance.snapshot`; bucket rules: `moves/` 2 days, `snapshots/` 14 days; deleted with the bot). A failed snapshot fails the rebuild before anything irreversible: the bot stays stopped on its old container, like a failed migration. Not exercised on prod yet (no image change today); `findOrCreateContainer`'s crash-recovery rebuild takes no snapshot. Restore by hand: `gcloud storage cp` the tar, copy it to the worker over IAP, `docker run --rm -v <volume>:/st alpine tar -xf - -C /st` with the bot stopped, and pin the image the snapshot was taken under.
- `sudo recreate-tenants --image <AGENT_RUNTIME_IMAGE> [--only <id|containerName>] [--force] [--keep-going] [--list]` on VM `orchestrator` (installed at `/usr/local/sbin`, source `infra/ops/recreate-tenants.sh`): rebuilds a bot that is off the image, fails a check, or with `--force`; waits for a heartbeat from after the call; verifies; stops at the first failure. `agent-forall-admin` now sends the JSON content-type only with a body (Fastify answers 400 to an empty JSON body).
- Found by the rehearsal: prod's bot image (`openclaw-browser@sha256:f0e4aec9`, built 2026-09-02) ships only the credit and media tarballs; the WhatsApp Business plugin is in the Dockerfile but no image with it was ever rolled. `verify` therefore requires that plugin only on a bot that has a `whatsapp_cloud` channel. Rolling a new bot image (it also carries the pending credit-plugin text) is a separate, customer-visible decision: test bot first with `--only`.
- Rehearsal on the test bot: `--list` reported the honest state, a forced rebuild took 1m43s and passed every check; 15/15, 0 warnings. 623 unit tests.

## Placement burst fix + browser tab cleanup (DEPLOYED 2026-09-25 14:57 UTC, orchestrator `83444a6e`, main 6703c61)
- Placement reserves each placed bot against its host until a memory sweep reads it (30 min backstop), releases it on a failed create, and picks + reserves without yielding. `PLACEMENT_OVERCOMMIT=2` in `control-plane.sh`: an unmeasured bot counts at half its limit (about 20 signups per 5-minute window instead of about 10).
- Browser tabs: every night in the 04:xx hour Asia/Jerusalem the orchestrator closes all page tabs on every running bot (`openclaw browser tabs/close`, 30 s CLI timeout); a bot over 80% of its memory limit gets its tabs closed at once, at most 3 attempts per episode, event `instance.browser_tabs_closed`. The browser process is never stopped, so logins survive. Logs: "browser tabs closed", "nightly browser tab cleanup" (info, VM only), "browser tab cleanup failed" (warn). Upstream: openclaw#138591 (tab idle timing hard-coded).
- First manual run 2026-09-25 on the oldest bot: 4 tabs closed, 3.78 → 2.06 GB, same Chrome pid.
- Alerts: disk and memory conditions use `EVALUATION_MISSING_DATA_INACTIVE`; the 16 + 52 closure emails on 2026-09-24/25 were the pre-filter loop-device incidents closing after the 7-day default.
