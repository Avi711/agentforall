...
**Changing tier** (`POST /api/billing/change-plan`, preview at `/change-plan/preview`): the existing subscription is updated at the provider, never replaced. A higher price or another interval is charged now, prorated (`prorated_immediately`, `on_payment_failure: prevent_change`: a declined card changes nothing, `payment_declined`); its `subscription_update` transaction arrives as `subscription.prorated`, is recorded as a payment with no `plan_code` (a renewal is checked against the last full charge of its own plan, falling back to the catalogue price) and grants `Σ quantity × rate × includedCredits` of its lines (a retired price resolves through its Paddle plan tag) under `plan:<provider>:<txn>`, so a refund revokes them. A cheaper plan is switched with `do_not_bill`: Paddle bills it from the next renewal, `scheduled_plan_code` holds it and `plan_code` keeps the paid plan until that renewal (a snapshot of the same period reporting the scheduled plan never overwrites it; a new period applies it even before the renewal charge lands); choosing the paid plan again drops the switch. Refused while overdue, ending (`subscription_ending`: resume first), on a yearly plan (support), within an hour of renewal or below Paddle's minimum charge (`renewal_imminent`). An upgrade while a switch is scheduled is a normal prorated change and drops the switch; Paddle credits only what it billed, so after a `do_not_bill` swap the current plan earns no credit and the charge covers the full rest of the period, with matching credits (shown before confirming).
