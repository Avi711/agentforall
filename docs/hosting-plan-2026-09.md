...
3. **Alerts.** Terraform: notification channel (Telegram via webhook, or email), uptime check on `https://api.agentforall.co.il/health`, VM memory > 85% for 5 min, wire the existing disk policies.
4. **Restore test.** Create a disk from yesterday's snapshot, mount on a scratch VM, open one tenant's SQLite. Document the steps.
