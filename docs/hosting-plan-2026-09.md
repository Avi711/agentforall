...
- Relay URLs derived from `ORCHESTRATOR_INTERNAL_URL` at render time; the per-bot stored copies removed by a one-shot migration. Today the env flip is a no-op for existing bots.
...
4. Remote runtime: one `DockerContainerRuntime` per host over mTLS; probes and RPC dial the worker's private IP; pairing sidecar port published on the VPC IP; per-host probe concurrency. Image GC lives here, not in cron: after a bot switches image the runtime removes images no container on that host references (Docker never does this itself; on 2026-09-14 the boot disk hit 75 % with 25 GB of superseded digests kept by the 7-day prune filter).
5. `move` as above, rehearsed on קוקי30 before any customer bot.
6. Terraform: dedicated VPC, NAT sizing, worker role + service account, control-plane CA, `worker_count`, label-keyed alerts and per-host log metrics. Workers boot from a prebuilt image (Docker, ops agent, runtime images baked in), so `startup.sh` for a worker is configuration only and a boot takes seconds, not four minutes. containerd's root goes on the data disk in that image: Docker 29 stores image layers in containerd, so `data-root` alone leaves them on the boot disk. Disk alerts per filesystem.
