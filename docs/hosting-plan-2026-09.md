...
2. Stable address `https://orchestrator.internal`.
   ...
   - **Pass 1 (pending the user's go):** `recreate-tenants.sh --force` so every bot carries the CA while still on http; the script asserts the relay through the CA per bot. **Pass 2:** flip `ORCHESTRATOR_INTERNAL_URL` to https + orchestrator restart; bots switch on their next config write. The orchestrator stays on `tenant-net` until every `openclaw.json` shows the https URL, and S-14 (Caddy on tenant-net serves the public site to bots) is closed before it leaves.
